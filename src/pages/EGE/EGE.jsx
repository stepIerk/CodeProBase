...
const topics = [
  {
    ...
    text: "Задания 16–17, 19–21, 23–27: пишем решения только на Python. Рекурсивные функции, обработка последовательностей и строк, теория игр, подсчёт маршрутов в графах, сложные задачи на сортировки и анализ данных. Учимся писать чистый и понятный код, а части решения при необходимости проверяем вручную по критериям ЕГЭ.",
  },
  {
    icon: Brain,
    title: "Логика и теория — тоже с Python",
    text: "Задания 2, 8, 11, 13, 14: логические выражения, таблицы истинности, объём информации, системы счисления, компьютерные сети. Показываю два подхода: классический аналитический и программный, решая эти задачи скриптами на Python для ускорения и надёжной проверки.",
  },
  {
    icon: Cpu,
    title: "Алгоритмизация и исполнители с кодом",
    text: "Задания 5, 16, 23–24: анализ алгоритмов, исполнители, рекурсия и маршрутные задачи. Разбираю логику руками, а затем пишем Python‑скрипты, которые моделируют исполнителей и проверяют гипотезы, чтобы не допускать ошибок в сложности вариантов.",
  },
  {
    icon: ClipboardCheck,
    title: "Поиск, таблицы",
    text: "Задания 9, 13, 27: поиск и фильтрация данных, электронные таблицы, работа с реляционными моделями. Учимся переносить идеи из Excel в Python: читаем данные, фильтруем и проверяем ответы программно.",
  },
];
...
const features = [
  {
    ...
    text: "Проходим каждую тему от простых задач до сложных номеров 24–27. Учимся выбирать оптимальный способ решения.",
  },
  {
    icon: Trophy,
    title: "Реальные пробники",
    text: "Регулярные пробные экзамены с замером времени и разбором ошибок. Привыкаем к формату и учимся распределять время.",
  },
  {
    icon: PenTool,
    title: "Акцент на программирование",
    text: "Большая часть баллов — за задачи с кодом. Учимся писать эффективные, читаемые и безошибочные решения на Python.",
  },
  {
    icon: BookOpen,
      ...
      <section className="container ege-hero">
        <motion.div className="ege-copy" variants={staggerContainer}>
          ...
          <motion.h1 className="section-title ege-title" variants={revealUp} custom={0.12}>
            Подготовка к ЕГЭ по информатике — <span>максимум баллов</span> без воды
          </motion.h1>
